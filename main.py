...
# makes several plans and returns the most effieicnt plan
def calculate_eurailpass(plan_data):
    origin_price = 0
    temp_set = {}
    eurailpass_set = []
    ...
    for day in eurailpass_price["global"]["youth"]["continuous"].keys():
        price = 0
        temp_set["date"] = int(day)
        temp_set["pass_price"] = eurailpass_price["global"]["youth"]["continuous"][day]['price'] * (100 - eurailpass_price["discounts"]) / 100
        temp_set["nations"] = []
        temp_set["pass_name"] = make_pass_name("global", "continuous", int(day))
        temp_set["pass_train"] = filter_global_continuous_gugan(plan_data, int(day))
        temp_set["gugan_train"] = difference(plan_data, temp_set["pass_train"])
        for gugan in temp_set["gugan_train"]:
            price += gugan['price']
        temp_set["gugan_price"] = price
    ...
    if (len(sorted_eurailpass_set) > 0) and (sorted_eurailpass_set[0]["total_price"] < origin_price):
        return sorted_eurailpass_set[0]
    else:
        temp_set["date"] = int(day)
        temp_set["pass_price"] = 0
        temp_set["pass_name"] = "null"
        temp_set["pass_train"] = []
        temp_set["gugan_train"] = plan_data
        temp_set["nations"] = []
        temp_set["gugan_price"] = origin_price
        temp_set["total_price"] = origin_price
...
def filter_global_not_continuous_gugan(plan_data, day, valid_days):
    ...
    sorted_plan_data = make_sorted_plan_data(plan_data, day)
    ...
    for i in range(len(sorted_plan_data)):
        if calculate_date(sorted_plan_data[i]['plan']) < valid_days:
            ...
            for plan in sorted_plan_data[i]['plan']:
                ...
                nations.add(plan['dst_nation'])
            if len(nations) <= 4:
                if check_neighboring_nation(nations):
                    return_data = sorted_plan_data[i]['plan']
...
def make_sorted_plan_data(plan_data, day):
    sort_list = []
    iter_list = []
    temp_list = []
    temp_dict = {}
    ...
    if len(plan_data) < day:
        combi_day = len(plan_data)
    else:
        combi_day = day

    for iteration in itertools.combinations(plan_data, combi_day):
...
def check_neighboring_nation(nations_list):
    ...
    for nation in nations_list:
        if(len(nations_list.difference(set(neighboring_nation[nation]))) is not 0):
            nations_flag = False
            break
